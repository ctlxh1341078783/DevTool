...
def main():
    # --cleanup 模式：由原卸载进程从 %TEMP% 启动，负责收尾删除
...
class UninstallerWindow:
    ...
    def _self_destruct(self):
        ...
        if IS_WIN:
            # 把自己复制到 %TEMP%，启动副本执行最终清理，自身退出
            tmp_exe = Path(tempfile.gettempdir()) / "_devtool_uninst_cleanup.exe"
            try:
            ...
            sh_path = Path(tempfile.gettempdir()) / "_devtool_uninst.sh"
            script = f'''#!/bin/bash
sleep 2
rm -rf "{install_dir}"
rm -f "$0"
